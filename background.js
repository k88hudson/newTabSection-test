...
const getData = async (forceBroadcast = false) => {
  ...
  if (!cards || dateString !== currentDate) {
    ...
    shuffle(cards);
    await browser.newTabSection.addCards(cards.slice(0, 20), forceBroadcast);
    console.log("DONE!");
  }
};

const onClick = action => {
  console.log("A site has been clicked!", action);
};
const onDismiss = action => {
  console.log("A site has been dismissed!", action);
};


browser.newTabSection.onClick.addListener(onClick);
browser.newTabSection.onDismiss.addListener(onDismiss);

browser.tabs.onCreated.addListener(tab => getData());
